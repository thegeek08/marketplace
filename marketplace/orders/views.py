...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        commission_rate = Decimal(str(getattr(django_settings, 'COMMISSION_RATE', 10)))
        commission_amount = (cart_total * commission_rate / Decimal('100')).quantize(Decimal('0.01'))
        ...
        audit_logger.info(
            'ORDER_CREATED | order_id=%d client_id=%d total=%s commission=%s payment=%s',
            order.pk, request.user.pk, order.total, order.commission_amount, order.payment_method
        )
