...
class RegisterForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if phone and role:
            ...
            existing_same_role = existing.filter(role=role).first()
            if existing_same_role:
                if existing_same_role.phone_verified:
                    # Compte vérifié → bloquer
                    role_label = "client" if role == "client" else "vendeur"
                    raise forms.ValidationError(
                        f"Un compte {role_label} existe déjà pour ce numéro de téléphone."
                    )
                # Compte non vérifié → stocker pour traitement dans la vue
                cleaned_data['existing_unverified_user'] = existing_same_role
