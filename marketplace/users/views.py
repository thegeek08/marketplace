...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            existing_unverified = form.cleaned_data.get('existing_unverified_user')
            threshold_24h = timezone.now() - timedelta(hours=24)

            # ── Compte non vérifié < 24h : renvoyer le code, rediriger vers vérif ──
            if existing_unverified and existing_unverified.created_at > threshold_24h:
                ...
                messages.info(
                    request,
                    "Un compte non vérifié existe avec ce numéro, un nouveau code vous a été envoyé."
                )
                audit_logger.info(
                    'REGISTER_RESEND | phone=%s role=%s ip=%s',
                    user.phone, user.role, get_client_ip(request)
                )
                return redirect("verify_phone")

            # ── Compte non vérifié > 24h : supprimer l'ancien et réinscrire ──
